...
`read.table(file = "path/dataset_name.txt", header = TRUE)` --> read in a text file

`write.table(data, file = "path/dataset_name.txt")` --> write a text file

`read.csv(file = "path/dataset_name.csv", header = TRUE)` --> read a csv file **(Most of the data sets you will be working with are csv files.)**

`write.csv(data, file = "path/dataset_name.csv")` --> write a csv file
