...
IDK where this would go --> `sum(dataset$variable)` --> adds a column (variable) in a data set
...
`model_name <- lm(dataset$y.var ~ dataset$x.var)` --> calculates a linear model
  `summary(model_name)` --> provides summary of the linear model
  `coef(model_name)` --> provides coefficient estimates
  `confint(model_name)` --> calculates confidence intervals for the regression coefficients
  `abline(model_name)` --> adds the estimated regression equation to existing scatterplot
