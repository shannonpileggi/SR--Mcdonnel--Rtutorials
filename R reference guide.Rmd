...
* not adding pt() and qt() right? the applet is better for that i think
...
* for lab 7 question 7, will students know to make a contingency table? or should i add this in the lab 7 section
...
**Requires you to install and load *mosaic* package first.**
...
`table(dataset$categ_var)` --> creates table you need for sample proportion
...
`table(dataset$var1, dataset$var2)` --> creates contingency table to find the two proportions

`prop.test(c(x1, x2), c(n1, n2), correct = FALSE)` --> performs a two sample proportion test (*z*-test)

* `x1` = number 'success' in group 1, `x2` = number success in groupn 2, `n1` = total number in group 1, `n2` = total number in group 2
* The default settings are to test a two sided alternative hypothesis and to calculate a 95% confidence interval.
* You can specify `alternative = "__"` and `conf.level = 0.__` to perform one sided tests and change the confidence level.
* The argument `correct = FALSE` overrides the continuity correction.
...
`table(dataset$var1, dataset$var2)` --> creates contingency table to examine the relationship between two variables

`chisq.test(dataset$var1, dataset$var2, correct=FALSE)` --> performs a chi-squared test of independence (when there are more than two groups to compare)

**Assumption of the Chi-squared test**: All expected cell counts should be at least 5.

* `results = chisq.test(dataset$var1, dataset$var2, correct=FALSE)` --> saves the chi-squared results
* `results$expected` --> prints the expected cell counts you need to check the test assumptions (all expected cell counts are at least 5)
